...
namespace {
    class CRevAddressVisitor : public boost::static_visitor<bool> {
    private:
        CRevAddress *addr;
    public:
        CRevAddressVisitor(CRevAddress *addrIn) : addr(addrIn) { }
...
bool CRevAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CRevAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CRevAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CRevAddressVisitor(this), dest);
}

bool CRevAddress::IsValid() const {
...
CTxDestination CRevAddress::Get() const {
    if (!IsValid())
...
bool CRevAddress::GetKeyID(CKeyID &keyID) const {
...
bool CRevAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CRevSecret::SetKey(const CKey& vchSecret) {
...
CKey CRevSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CRevSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CRevSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CRevSecret::SetString(const std::string& strSecret) {
