    ...
    nWeight(0)
{
    ...
    setWindowTitle(tr("Rev") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    ...
    setObjectName("Rev");
    setStyleSheet("#Rev { background-image: url(:/images/background_light); }");//background-color: #e6e6e6; color: #333333;
...
void RevGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Rev address"));
    ...
    aboutAction = new QAction(QIcon(fUseDarkTheme ? ":/icons/dark/bitcoin-dark" : ":/icons/bitcoin"), tr("&About Rev"), this);
    aboutAction->setToolTip(tr("Show information about Rev"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Rev"));
    ...
    editConfigAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Rev.conf"), this);
    editConfigAction->setToolTip(tr("Edit the configuration file for Rev"));
    editConfigExtAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Rev.conf (external)"), this);
    editConfigExtAction->setToolTip(tr("Edit the configuration file for Rev (external editor)"));
    openDataDirAction = new QAction(QIcon(":/icons/folder"), tr("&Open data dir"), this);
    openDataDirAction->setToolTip(tr("Open the directory where Rev data is stored"));
...
void RevGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolBar { background: none; } QToolButton { color: #ffffff; background: none; } QToolButton:hover { background-color: rgba(217, 199, 113, 0.4); } QToolButton:checked { background-color: rgba(217, 199, 113, 0.4); } QToolButton:pressed { background-color: rgba(217, 199, 113, 0.4); } #tabs { color: #ffffff; background: none; border: none;}");
    toolbar->setIconSize(QSize(64,64));

    if(!fUseDarkTheme)
    {
        toolbar->setStyleSheet("QToolBar { background: none; } QToolButton { color: #ffffff; background: none;} QToolButton:hover { background-color: rgba(217, 199, 113, 0.4); } QToolButton:checked { background-color: rgba(217, 199, 113, 0.4); } QToolButton:pressed { background-color: rgba(217, 199, 113, 0.4); } #tabs { color: #ffffff; background: none; border: none;}");//QToolBar { background: none; }
    }
...
void RevGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Rev client") + QString(" ") + tr("[testnet]"));
...
void RevGUI::setWalletModel(WalletModel *walletModel)
{
...
void RevGUI::setMessageModel(MessageModel *messageModel)
{
...
void RevGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Rev client"));
...
#ifndef Q_OS_MAC
void RevGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void RevGUI::aboutQtExt_Internal()
{
    aboutQtAction->triggered();
}

void RevGUI::aboutQtExt_Static()
{
    guiref->aboutQtExt_Internal();
}

void RevGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    //labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Rev network", "", count));
}

void RevGUI::setNumBlocks(int count)
{
    QString tooltip;
...
void RevGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Rev") + " - ";
...
void RevGUI::error(const QString &title, const QString &message, bool modal)
{
...
void RevGUI::changeEvent(QEvent *e)
{
...
void RevGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void RevGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(RevUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void RevGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    notificator->notify(Notificator::Information,
                        (amount)<0 ? tr("Sent transaction") :
                          ...
                          tr("Date: %1\n"
                             ...
                             "Address: %4\n")
                          .arg(date)
                          .arg(RevUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                          .arg(type)
                          .arg(address), icon);
}

void RevGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void RevGUI::gotoMasternodeManagerPage()
{
...
void RevGUI::gotoOverviewPage()
{
    ...
    setStyleSheet("#Rev { background-image: url(:/images/background_light); }");
}

void RevGUI::gotoHistoryPage()
{
...
void RevGUI::gotoHistoryPage_static()
{
    guiref->gotoHistoryPage();
}

void RevGUI::gotoAddressBookPage()
{
...
void RevGUI::gotoReceiveCoinsPage()
{
...
void RevGUI::gotoSignMessageTab(QString addr)
{
...
void RevGUI::gotoVerifyMessageTab(QString addr)
{
...
void RevGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RevGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Rev address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void RevGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void RevGUI::setWalletUnlockStakingOnly()
{
...
void RevGUI::setEncryptionStatus(int status)
{
...
void RevGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void RevGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RevGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void RevGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

/*void RevGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
