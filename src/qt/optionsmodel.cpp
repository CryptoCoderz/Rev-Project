...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", RevUnits::REV);
    ...
    if (!settings.contains("nAnonymizeRevAmount"))
        settings.setValue("nAnonymizeRevAmount", 1000);
    nAnonymizeRevAmount = settings.value("nAnonymizeRevAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeRevAmount"))
        SoftSetArg("-anonymizeRevamount", settings.value("nAnonymizeRevAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeRevAmount:
            return QVariant(nAnonymizeRevAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeRevAmount:
            nAnonymizeRevAmount = value.toInt();
            settings.setValue("nAnonymizeRevAmount", nAnonymizeRevAmount);
            emit AnonymizeRevAmountChanged(nAnonymizeRevAmount);
            break;
        default:
            break;
        }
