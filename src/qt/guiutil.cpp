...
{
    widget->setMaxLength(RevAddressValidator::MaxAddressLength);
    widget->setValidator(new RevAddressValidator(parent));
...
bool parseRevURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("Rev"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!RevUnits::parse(RevUnits::REV, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseRevURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to Rev:
    //
    ...
    {
        uri.replace(0, 11, "Rev:");
    }
    QUrl uriInstance(uri);
    return parseRevURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Rev.lnk";
}
...
{
    return GetAutostartDir() / "Rev.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Rev.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Rev app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Rev app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Rev-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Rev-qt [" + tr("command-line options") + "]                     " + "\n";
