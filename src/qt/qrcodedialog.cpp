...
{
    QString ret = QString("Rev:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non REV unit input in lnReqAmount, we generate the URI with REV as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(RevUnits::format(RevUnits::REV, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
